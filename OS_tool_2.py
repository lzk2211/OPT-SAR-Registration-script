...
class MainWindow(QMainWindow):
    ...
    def save_points_to_csv(self):
        ...
        with open(save_path, mode='w', newline='') as file:
            ...
            writer.writerow(["filename", "index", "x1", "y1", "x2", "y2"])
            for i, (p1, p2) in enumerate(zip(self.image_label1.points, self.image_label2.points)):
                writer.writerow([filename, i, int(p1.x()), int(p1.y()), int(p2.x()), int(p2.y())])
    ...
    #     if self.image_label1.image and self.image_label2.image:
    #         zoom_factor = 1.25 if event.angleDelta().y() > 0 else 0.8
    #         self.image_label1.scale_factor *= zoom_factor
    #         self.image_label2.scale_factor *= zoom_factor
    #         self.image_label1.update_display()
    #         self.image_label2.update_display()
